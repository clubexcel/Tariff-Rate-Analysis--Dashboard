...
# --- 7. Interactive Regression ---
st.divider()
st.header("🧮 Custom Regression Explorer")
st.info("Explore relationships between different financial metrics dynamically.")

if df_banks is not None:
    numeric_cols = df_banks.select_dtypes(include=np.number).columns.tolist()
    
    c1, c2, c3 = st.columns([1, 1, 2])
    x_var = c1.selectbox("Predictor (X Axis)", numeric_cols, index=0)
    y_var = c2.selectbox("Target (Y Axis)", numeric_cols, index=1)
    
    if x_var and y_var:
        sub_df = df_banks[[x_var, y_var]].dropna()
        if len(sub_df) > 1:
            X = sub_df[[x_var]].values
    ...
    st.error("Banking data not available for regression analysis.")
